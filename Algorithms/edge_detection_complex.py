...
def mark_corners(image, max_corners=100, canny_threshold1=50, canny_threshold2=150):
    """
    Detect and mark corners of shapes in an image with blue pins, improved for complex images.
    ...
    # Dilate the edge image to enhance edge lines
    kernel = np.ones((3,3), np.uint8)
    dilated_edges = cv2.dilate(edges, kernel, iterations=1)

    # Detect corners on dilated edge image
    corners = cv2.goodFeaturesToTrack(dilated_edges, maxCorners=max_corners, qualityLevel=0.01, minDistance=10)
